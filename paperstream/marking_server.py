...
WEB_ANSWER_AREA_PATH = "static/template.png"
...
class TemplateResource(object):
    def on_get(self, req, resp):
        ...
        # If there is at least one tiff file in the template dir
        if len(tif_files) > 0:
            template_path = tif_files[0]
            try:
                template_answer_area = encode.get_first_page_answer_area(
                    template_path)
                cv2.imwrite(WEB_ANSWER_AREA_PATH, template_answer_area)
...
class ScannedDiariesResource(object):
    def on_get(self, req, resp):
        ...
        resp.set_header('Content-Type', 'text/json')
        diaries_paths = encode.get_files_in_directory(
            encode.DIARIES_TO_ENCODE_DIR, ".tif")
...
class PDFTemplateDiariesResource(object):
    def on_get(self, req, resp):
        ...
        resp.set_header('Content-Type', 'text/json')
        diaries_paths = encode.get_files_in_directory(
            encode.DIARIES_TO_CREATE_DIR, ".pdf")
...
class EncodeResource(object):
    def on_post(self, req, resp):
        """
        Encodes a diary (tif file) based on a rubric (created in the web interface) and a blank 
        page of the diary (tif file) present in TEMPLATE_DIR
        """
        resp.set_header('Content-Type', 'text/json')
...
class CreateResource(object):
    def on_post(self, req, resp):
        ...
        try:
            ...
            starting_date = create.valid_date(starting_date)

            created_diary = create.create_diary(
                pdf_template, pages, starting_date, email, font)
            resp.body = json.dumps(created_diary)

            print("Diary Created: " + json.dumps(created_diary))
        except Exception as e:
            print(str(type(e)))
            print(str(e))
            print(traceback.print_tb(e.__traceback__))
...
class DownloadFilesResource(object):
    def compress_files(files, zip_name):
        ...
        with zipfile.ZipFile(resource_path(zip_path), 'w') as myzip:
            for file in files:
                base_folder = os.path.basename(os.path.dirname(file))
